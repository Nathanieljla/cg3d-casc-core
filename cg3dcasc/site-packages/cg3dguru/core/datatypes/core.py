...
class PyScene(SceneElement):
    ...
    def get_scene_objects(self, names = [], selected = False, of_type = '', only_roots = False) -> typing.List[PyObjectId]:
...
class PyBehaviour(PyGuid):
    ...
    def _get_dynamic_name(self):
        #if True:
            #if self. is_dynamic:
                #print ('{} {}'.format(self.object.name, 'Dynamic'))
            #return self._name
    ...
    def _get_data(self, prop_name, is_range):
        ...
        if is_range:
            ...
            result = DataRange(prop_name, value, self)
        else:
            value = self.get_data(prop_name)
            result = DataProperty(prop_name, value, self)
...
class PropertyRange(Property):
    def __init__(self, name, entries: list, behaviour):
        super(PropertyRange, self).__init__(name, None, behaviour)

        wrapper = self._get_wrapper_class()
        self._content = [wrapper(self._get_item_name(item, behaviour), item, behaviour) for item in entries]        


    def _get_wrapper_class(self):
        """Returns the property class the list contains"""
        
        raise NotImplementedError
    
    
    def _get_item_name(self, item, behaviour):
        """Return the name of element in the list"""
...
class DataRange(PropertyRange):
    ...
    def _get_item_name(self, item, behaviour, *args):
        return behaviour.dat_viewer.get_data(item).name
    

    def get_by_name(self, data_name, case_sensitive=False) -> typing.List[DataProperty]:
        """Returns a list of data properties that match the input name"""
        
        if not case_sensitive:
            return [element for element in self._content if element.name.lower() == data_name.lower()]
        else:
            return [element for element in self._content if element.name == data_name]            
    
    
    def add(self, element: DataProperty):
        """Create a new csc.model.Data and assign it to the property"""
        added = False
        def _add_element():
            nonlocal added
            added = self.beh_editor.add_behaviour_data_to_range(self.behaviour, self.name, element)
           
        self.scene.edit("Add data to {}".format(self.name), _add_element, _internal_edit=True)
        return added
    ...
    def create_data(self, name: str, mode: csc.model.DataMode, value, data_id=None, group_name=None):
        ...
        def _create_data():
            ...
            if new_data is not None:
                ...
                print('failed to create and add data to {}'.format(self.name))
    ...
    def remove(self, element):
        #erase_behaviour_data_from_range
        pass
    
    
    def set(self, data_range):        
        #set_behaviour_data_to_range
        pass
...
class DataProperty(Property):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__data = None #don't confuse this with the wrapped data self._data
        if self.get_content():
