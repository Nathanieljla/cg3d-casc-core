...
class CscWrapper(object):
    ...
    def wrap(data: object, creator: object) -> CscWrapper: #, default_class = None) -> CscWrapper:
        ...
        class_mapping = {
            ...
            csc.domain.Selector: CscWrapper,
            csc.domain.SelectionChanger: CscWrapper,
...
class PyScene(SceneElement):
    ...
    def select(self, *args, **kwargs):
        def _select(): 
            self.session.take_selector().select(*args, **kwargs)
        ...
        def _add_behaviour():
            ...
            if add_dynamic:
                ...
                if name_prop:
                    ...
                    output.set_string('behaviourName', dynamic_name)
...
class PyBehaviour(PyGuid):
    ...
    def _get_data(self, prop_name, is_range):
        ...
        if is_range:
            result = DataRange(prop_name, self)
        else:
    ...
    def _get_setting(self, prop_name, is_range):
        ...
        if is_range:
            result = SettingRange(prop_name, self)
        else:
            value = self.get_setting(prop_name)
            result = SettingProperty(prop_name, value, self)
    ...
    def _get_object(self, prop_name, is_range):
        ...
        if is_range:
            result = ObjectRange(prop_name, self)
        else:
            value = self.get_object(prop_name)
            result = ObjectProperty(prop_name, value, self)
    ...
    def _get_reference(self, prop_name, is_range):
        ...
        if is_range:
            result = ReferenceRange(prop_name, self)
        else:
            value = self.get_reference(prop_name)
            result = ReferenceProperty(prop_name, value, self)
    ...
    def get_property(self, name) -> DataProperty | ObjectProperty |\
        ...
        if PropertyType.DATA in property_type:
            ...
            print("cant get property type for {} found as {}".format(name, property_type))
            prop = None
...
class PropertyRange(Property):
    def __init__(self, name, behaviour): #entries: list, behaviour):
...
class DataRange(PropertyRange):        
    ...
    def _wrap_item(self, item) -> DataProperty:
        ...
        return DataProperty(name, item, self.behaviour)
...
class SettingRange(PropertyRange):        
    ...
    def _wrap_item(self, item) -> DataProperty:
        ...
        return SettingProperty(name, item, self.behaviour)
